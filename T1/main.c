...
int main (int argc, char** argv) {
	...
	aeroporto_t* aeroporto_inicial = iniciar_aeroporto(args, n_args);
	meu_aeroporto = aeroporto_inicial;
	...
	pthread_t contador_threads[1+TEMPO_SIMULACAO/30];

	int id=0;
	int tempo_total = (int)t_simulacao;
	while(tempo_total > 0){
		tempo_t prox_aviao = (rand() % t_novo_aviao_max) + t_novo_aviao_min;
		if(tempo_total >= prox_aviao){
			int combustivel = (rand() % p_combustivel_max) + p_combustivel_min;
			aviao_t* aviao = aloca_aviao(combustivel, id);
			
			pthread_create(&contador_threads[id++],NULL, thread_aviao, (void*) aviao);

			usleep(prox_aviao * 1000);
		}	
		tempo_total-=prox_aviao;		
	}

	for (int i = 0; i < id; i++)
	{
		pthread_join(contador_threads[i], NULL);
	}
...
void * thread_aeroporto_func(void* arg) {
	...
	while (1) { 
		pthread_mutex_lock(&aeroporto->requer_pista);
		pthread_mutex_unlock(&aeroporto->requer_pista);
		sem_wait(&aeroporto->pistas);
		if (aeroporto->fila_preferencial->n_elementos > 0)
			sem_post(&aeroporto->fila_preferencial->primeiro->dado->liberacao_pouso_decolagem);
		else
			sem_post(&aeroporto->fila_pouso->primeiro->dado->liberacao_pouso_decolagem);
	}
